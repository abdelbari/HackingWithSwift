...
enum PhotoPalette {
    ...
    static func extract(from image: UIImage, count: Int = 6,
        ...
        return extract(from: cg, count: count, minDistance: minDistance)
    }

    static func extract(from cg: CGImage, count: Int = 6, minDistance: Double = 48) -> [String] {
        guard count > 0, let pixels = samples(of: cg) else { return [] }
    ...
    /// The picture at 64 pixels on its longer side, as straight RGBA bytes.
    private static func samples(of cg: CGImage, maxEdge: Int = 64) -> [UInt8]? {
        let scale = Double(maxEdge) / Double(max(cg.width, cg.height, 1))
